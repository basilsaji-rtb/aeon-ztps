...
from aeon.rtbrick.device import Device
from paramiko import AuthenticationException
from paramiko.ssh_exception import NoValidConnectionsError
from aeon.exceptions import LoginNotReadyError
from aeon.rtbrick.accton import RtBrick_AS7712
...
def cli_parse(cmdargs=None):
    psr = argparse.ArgumentParser(
        prog='rtbrick_bootstrap',
        description="Aeon-ZTP bootstrapper for RtBrick BrickOS",
        add_help=True)
        ...
        while not dev:
            ...
            try:
                dev = Device(self.target, user=self.user, passwd=self.passwd,
                             timeout=poll_delay)
  		print "Trying to loginto {} with user {} and password {}".format(self.target, self.user, self.passwd)
  		...
  		print "Connection error"
                ...
                if countdown <= 0:
                    self.exit_results(results=dict(
                        ...
                        error_type='login',
                        message='Failed to connect to target %s within reload countdown' % self.target))
  		print "Login Not ready error"

                time.sleep(poll_delay)

        self.dev = dev
        self.post_device_facts()
        rdev = RtBrick_AS7712(self.target, self.server)
        rdev.config()
...
def main():
    ...
    cboot = RtbrickBootstrap(self_server, cli_args)
